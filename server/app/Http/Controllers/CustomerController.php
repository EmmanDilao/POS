...
class CustomerController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            $validator = Validator::make($request->all(), [
                ...
                'phone_number' => 'required|string|max:11|min:11|unique:customers,phone_number',
                'zip_code' => 'required|max:6|min:4',
            ]);
            if ($validator->fails()) {
            ...
            DB::commit();
            return $this->sendResponse("customer created succesfully.", $data, 201);
        } catch (Exception $e) {
            DB::rollBack();
            return $this->handleException($e);
        }
    ...
    public function show(string $id): JsonResponse
    {
        try {
            ...
            return $this->sendResponse("customer fetch succesfully.", $data, 200);
        } catch (\Exception $e) {
            return $this->handleException($e);
        }
    }
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'phone_number' => 'required|string|max:11|min:11|unique:customers,phone_number,' . $id,
                'zip_code' => 'required|max:6|min:4',
            ]);
            if ($validator->fails()) {
                return $this->sendError("Please enter valid input data", $validator->errors(), 400);
            ...
            return $this->sendResponse("customer updated succesfully.", $data, 201);
        } catch (\Exception $e) {
            DB::rollBack();
            return $this->handleException($e);
        }
    ...
    public function destroy(string $id): JsonResponse
    {
        try {
            ...
            if (empty($data['customer'])) {
                ...
                DB::commit();
                return $this->sendResponse("customer deleted succesfully.", $data, 200);
            }
        } catch (\Exception $e) {
            DB::rollBack();
            return $this->handleException($e);
